...
router.get('/walkers/summary', async (req, res) => {
    try {
        const [ws] = await db.execute(`SELECT u.username AS walker_username, COUNT(r.rating) AS total_ratings, CAST(ROUND(AVG(r.rating), 1) AS FLOAT) AS average_rating, COUNT(w.request_id) AS completed_walks
                                        FROM Users AS u
